function loadTileMap (tilemap) {
    ...
    for(var l in tilemap.layers) {
        //alert("Doing layer "+tilemap.layers[l].name);
            ...
            if (!gidUsed(gid,usedGids)) {
                for(var i = tilemap.tilesets.length-1; i >= 0; --i) {
                    ...
                    if(tileset.firstgid <= gid) {
                        ...
                        var tilebase = PIXI.BaseTexture.fromImage(tileset.image);
                        var frame = { 
...
function getAcidArray(tilemap) {
    var acidArray = new Array(tilemap.width*tilemap.height);
    for(var i = 0; i < tilemap.width*tilemap.height; ++i) acidArray[i] = 0;
    for(var z in tilemap.acidspray.entities) {
        var spray = tilemap.acidspray.entities[z];
        acidArray[spray.ngrid_y*tilemap.width+spray.ngrid_x] = 1;
        acidArray[(spray.ngrid_y-1)*tilemap.width+spray.ngrid_x-1] = 1;
        acidArray[(spray.ngrid_y-1)*tilemap.width+spray.ngrid_x] = 1;
        acidArray[(spray.ngrid_y-1)*tilemap.width+spray.ngrid_x+1] = 1;
        acidArray[spray.ngrid_y*tilemap.width+spray.ngrid_x-1] = 1;
        acidArray[spray.ngrid_y*tilemap.width+spray.ngrid_x+1] = 1;
        acidArray[(spray.ngrid_y+1)*tilemap.width+spray.ngrid_x-1] = 1;
        acidArray[(spray.ngrid_y+1)*tilemap.width+spray.ngrid_x] = 1;
        acidArray[(spray.ngrid_y+1)*tilemap.width+spray.ngrid_x+1] = 1;
    }
    return { width: tilemap.width, height: tilemap.height, array: acidArray };
}
